...
| **PLDM 벤치** (`-G`) | requester(libpldm): DUT PLDM 디스커버리(GetTID/Types/Version/Commands) + FRU 테이블 읽기·**필드 디코드**(제조사/모델/부품번호/시리얼 등) / responder: 우리 PLDM Base 단말 자가검증 | requester ✅ / responder ❌ |
| **PLDM Platform** (`-T`) | DUT의 PDR 저장소를 GetPDRRepositoryInfo/GetPDR로 순회해 센서·이펙터를 열거하고, 각각에 GetSensorReading/GetStateSensorReadings/GetSensorThresholds/GetStateEffecterStates를 수행. `-W`를 주면 Set\*(0x10/0x13/0x39)를 **읽은 값 그대로 되쓰는 라운드트립**으로 검증(상태 보존) | ✅ 실제 버스 |
