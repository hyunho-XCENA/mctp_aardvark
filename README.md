...
| [`pldm_negative_test.c`](pldm_negative_test.c) | PLDM negative/conformance validator (잘못된 요청을 DUT가 올바른 에러코드로 거부하는지) |
| [`pldm_update_test.c`](pldm_update_test.c) | PLDM firmware update Update Agent (`.pldm` 패키지로 DUT 펌웨어 실제 flash) |
...
./run_mctp_test.sh -N -C         # PLDM negative/conformance (잘못된 요청 거부 검증)
...
./run_mctp_test.sh -F appli.pldm -C   # PLDM 펌웨어 패키지로 DUT를 실제 업데이트 (firmware WRITE!)
...
| **NEGATIVE** (`-N`) | 잘못된 요청(미지원 타입/커맨드, 잘못된 sensor/effecter ID, 잘못된 record handle, 잘린 페이로드)을 보내 DUT가 올바른 PLDM 완료코드로 거부하는지 검증. **잘못된 입력에 cc=SUCCESS를 주면 FAIL**(검증 안 한 버그). DUT 상태 안 바뀜 | ✅ 실제 버스 |
| **FW UPDATE** (`-F pkg`) | `.pldm` 패키지를 파싱해 DUT(Firmware Device)에 **실제 펌웨어를 flash**: RequestUpdate→PassComponentTable→UpdateComponent→[FD 주도 RequestFirmwareData/Transfer/Verify/ApplyComplete 서빙]→ActivateFirmware. ⚠️ **firmware WRITE** | ✅ 실제 버스 |
...
| `-N` | PLDM negative/conformance (잘못된 요청 거부 검증) | - |
...
| `-F pkg` | `.pldm` 패키지로 DUT 펌웨어 실제 업데이트 (firmware WRITE) | - |
